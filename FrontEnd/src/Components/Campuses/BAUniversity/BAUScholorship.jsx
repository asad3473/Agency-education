  ...
  const scholarships = [
    {
      ...
      description: isArabic
        ? "تُمنح لجميع الطلاب الدوليين المسجلين في المرحلة الجامعية في BCU"
        : "Granted to all international undergraduate students registering at BCU",
    ...
    },
    {
      ...
      description: isArabic
        ? "خصم 25% لكل شقيق يحضر (مطلوب إثبات القرابة)"
    ...
    },
    {
      ...
      description: isArabic
        ? "خصم 25% للأزواج المتزوجين الذين يحضرون معًا (مطلوب إثبات الزواج)"
    ...
    },
    {
      ...
      description: isArabic
        ? "للطلاب الذين لديهم أداء رياضي على المستوى الوطني أو المنضمين إلى فرق الرياضة في BCU"
        : "For students with national-level sports performance or joining BCU sports teams",
    ...
    },
    {
      ...
      description: isArabic
        ? "للطلاب الخريجين الذين لديهم معدل تراكمي لا يقل عن 3.50 (مطلوب شهادة بكالوريوس صالحة)"
    ...
    <div className={`max-w-7xl mx-auto px-4 py-12`} dir={isArabic ? 'rtl' : 'ltr'}>
      <div className="text-center mb-12">
        ...
        <p className="text-xl text-gray-600 max-w-3xl mx-auto">
          {isArabic
            ? "تقدم BCU دعمًا ماليًا سخيًا لمساعدة الطلاب الدوليين على تحقيق أحلامهم الأكاديمية"
            : "BCU offers generous financial support to help international students achieve their academic dreams"}
        </p>
      </div>
          ...
          >
            <div className="p-6">
              <div className="flex items-center mb-4">
                <div className="mr-4">{scholarship.icon}</div>
      ...
      <div className="mt-12 bg-gradient-to-r from-[#0A0851] to-blue-600 rounded-xl p-8 text-white">
        <h2 className="text-2xl font-bold mb-4">
          {isArabic ? "كيفية التقديم للمنح الدراسية" : "How to Apply for Scholarships"}
        </h2>
        <ul className="list-disc pl-5 space-y-2">
          <li>{isArabic ? "قدم طلبك للالتحاق" : "Submit your application for admission"}</li>
          <li>{isArabic ? "تضمين جميع المستندات الأكاديمية المطلوبة" : "Include all required academic documents"}</li>
          <li>{isArabic ? "للمنح الخاصة، قدم الأدلة الداعمة" : "For special scholarships, provide supporting evidence"}</li>
          <li>{isArabic ? "يتم النظر في المنح تلقائيًا عند التقديم" : "Scholarships are automatically considered upon application"}</li>
        </ul>
        <button className="mt-6 px-6 py-3 bg-white text-[#0A0851] font-bold rounded-lg hover:bg-gray-100 transition-colors">
          {isArabic ? "قدِّم الآن" : "Apply Now"}
        </button>
      </div>
    </div>
  );
